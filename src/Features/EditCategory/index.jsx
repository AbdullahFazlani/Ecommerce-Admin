...
const EditCategory = ({ row, refetch }) => {
  ...
  const {
    mutateAsync: updateCategoryMutate,
    isLoading: updateCategoryIsLoading,
    isError: updateCategoryIsError,
    data: updateCategoryRes,
    error: updateCategoryErr,
  } = UpdateCategoryData();
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      updateCategoryMutate({ updatedId, categoryData });
  ...
  return (
    <Box p={2}>
      ...
      <Box display="flex" justifyContent="flex-end">
        ...
        >
          {updateCategoryIsLoading ? <CircularProgress /> : "Update"}
        </Button>
      </Box>
    </Box>
  );
